...
public class Connexion extends JDialog implements WindowListener {
    ...
    private void b_connexionActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_b_connexionActionPerformed
        ...
        donnees.setPseudo(t_nom.getText());
        
        if (parent.getBDD().verifNomUtilisateur(donnees.getPseudo()))
        {
            int choix = JOptionPane.showConfirmDialog(this,"Le pseudo " + donnees.getPseudo() + " existe déjà dans la base de données, voulez-vous continuer","Question",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE);
            if( choix == JOptionPane.YES_OPTION )
            {
                    parent.setPseudo(donnees.getPseudo());
                    dispose();
            }
        ...
        {
            parent.getBDD().ajouterUtilisateur(donnees.getPseudo());
            parent.setPseudo(donnees.getPseudo());
            JOptionPane.showMessageDialog(this,"Le pseudo " + donnees.getPseudo() + " a été crée");
            dispose();
        }
